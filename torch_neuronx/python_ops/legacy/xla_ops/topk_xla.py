    ...
    def can_handle(
        ...
        **kwargs,
        ...
        if not super().can_handle(input, k=k, dim=dim, largest=largest, sorted=sorted, **kwargs):
            return super().can_handle(input, k=k, dim=dim, largest=largest, sorted=sorted, **kwargs)

        # k must be positive integer
        if not isinstance(k, int) or k <= 0:
            raise RuntimeError(f"selected index k invalid ({k=}, {type(k)})")
        ...
        if k > input.shape[dim]:
            raise RuntimeError(f"selected index k out of range ({k=}, {input.shape[dim]=})")

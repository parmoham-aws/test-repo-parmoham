...
class TestConv1dBackwardRegistration:
    """Test cases for conv1d backward operation"""
    ...
    def test_conv1d_run_on_neuron(self, config, input_shape, op_name):
        ...
        weight = torch.randn(
            out_channels, in_channels // groups, kernel_size, dtype=torch.float32
        ).to("neuron")
        bias = (
        ...
        l_out = ((input_shape[-1] + 2 * padding - dilation * (kernel_size - 1) - 1) // stride) + 1
        grad_output = torch.randn(input_shape[0], out_channels, l_out, dtype=torch.float32).to(
            "neuron"
        )
    ...
    def test_conv1d_fwd_bwd_basic(self):
        """Test forward-backward pass using loss.backward()"""

        input = torch.randn(2, 3, 10, dtype=torch.float32, requires_grad=True).to("neuron")
        input.retain_grad()
        conv = torch.nn.Conv1d(3, 2, 3, padding=1).to("neuron")
        target = torch.randn(2, 2, 10, dtype=torch.float32).to("neuron")
    ...
    def test_conv1d_cpu_match(self, config, input_shape, op_name, dtype, atol, rtol):
        # Test if gradients match between cpu and neuron

        def run_conv1d_backward(device):
            torch.manual_seed(0)
            ...
            out_channels = config["out_channels"]
            groups = config.get("groups", 1)
            weight = torch.randn(out_channels, in_channels // groups, kernel_size, dtype=dtype)
            ...
            grad_output = torch.randn(input_shape[0], out_channels, l_out, dtype=dtype)
        ...
        # Run on both devices
        with track_neuron_ops():
            neuron_grad_input, neuron_grad_weight, neuron_grad_bias = run_conv1d_backward("neuron")
            assert_op_runs_on_neuron("convolution_backward")
        neuron_grad_input, neuron_grad_weight, neuron_grad_bias = run_conv1d_backward("neuron")
        cpu_grad_input, cpu_grad_weight, cpu_grad_bias = run_conv1d_backward("cpu")
    ...
    def test_conv1d_output_mask(self, output_mask):
        """Test convolution_backward with different output_mask combinations"""

        def run_with_mask(device):
            torch.manual_seed(0)
            input = torch.randn((2, 3, 10), dtype=torch.float32, requires_grad=True)
            weight = torch.randn(2, 3, 3, dtype=torch.float32)
            bias = torch.randn(2, dtype=torch.float32)
            grad_output = torch.randn(2, 2, 10, dtype=torch.float32)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                RuntimeError,
                "Given groups=2, expected weight to be divisible by 2.*but got weight of size.*3",
            ),
            (
                ...
                RuntimeError,
                "Expected input and grad_output to have the same number of dimensions.*got.*3.*2",
            ),
            (
    ...
    def test_conv1d_backward_validation_errors(
